...
const Chat = () => {
  const [state, dispatch] = useReducer(reducer, { messages: [], users: [], error: null, isPending: false });
  ...
  return (
    <section>
      <h2>Chat</h2>
      {
        state.messages.map((msg) => <article key={msg.id}><h3>{msg.body}</h3><p>{msg.author.name}</p></article>)
      }
    </section>
  );
}
